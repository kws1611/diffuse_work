...
class Parser(Tap):
    ...
    # Rename set_seed to apply_seed to avoid conflicts
    def apply_seed(self, args):
        if not hasattr(args, 'seed') or args.seed is None:
            print('[ utils/setup ] No seed provided, setting a default seed value: 42')
            args.seed = 42  # Default seed value
        else:
            print(f'[ utils/setup ] Setting seed: {args.seed}')
        global_set_seed(args.seed)  # Call the global function here
    ...
    def parse_args(self, experiment=None):
        ...
        self.apply_seed(args)  # Use the renamed apply_seed method
    ...
    def read_config(self, args, experiment):
        ...
        # Print out all available keys in 'base'
        print(f'[ utils/setup ] Available keys in base configuration: {list(getattr(module, "base").keys())}')
        
        # Check if 'experiment' exists in 'base'
        if experiment not in getattr(module, 'base'):
            raise KeyError(f"Experiment '{experiment}' not found in base configuration. Available keys: {list(getattr(module, 'base').keys())}")

        params = getattr(module, 'base')[experiment]

        # Check if 'dataset' is present and if overrides are available
        ...
        # Populate args with config parameters
        self._dict = {}
    ...
    def set_seed(self, args):
        if not hasattr(args, 'seed') or args.seed is None:
            ...
            print(f'[ utils/setup ] Setting seed: {args.seed}')
        global_set_seed(args.seed)  # Use the renamed global set_seed function
    ...
    def mkdir(self, args):
        if 'logbase' in dir(args) and 'dataset' in dir(args) and 'exp_name' in dir(args):
            ...
            # Save the args to a specific file in the directory
            args_file_path = os.path.join(args.savepath, 'args.json')
            self.save(args_file_path)  # Save the arguments to the file
